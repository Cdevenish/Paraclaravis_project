...
# to load in...
# load("gis/r_sir/predNames.rdata")
# pred <- brick("gis/r_sir/predStack.tif")
# names(pred) <- pred.names
# pred.names
# 
# ## pca wc ####
# wc <- values(dropLayer(pred, ))


### 4. mapBiomas 
# 
# load("extents_aoi_dec2019.rdata")
# 
# ## Load all into R - multiband image - load as stack
# dir("gis/r_wgs/mapBiomas")
# mB.stck <- brick("gis/r_wgs/mapBiomas/mapbiomas_aoi.tif")
# mB.stck
# 
# ## Transform to sirgas and crop to extent at same time:
# msk.aoi
# beginCluster()
# mB.sir <- projectRaster(mB.stck, to = msk.aoi, method = "ngb")
# 
# #reclassify 0 to NA (these are NA values) and also 27 to NA (non observed)
# mB.rc <- clusterR(mB.sir, reclassify, args = list(rcl= c(-Inf, 0, NA,26,27,NA ), right=T))
# 
# endCluster()
# 
# 
...
# 
# names(mB.rc)
# names(mB.rc) <- sprintf("mB%d", 1985:2017)
# 
# mB.rc <- mask(mB.rc, mask= msk.aoi)
# 
# writeRaster(mB.rc, filename = "gis/r_sirg/mapBiomas.tif", bylayer = F, datatype = "INT4S", overwrite = T)
# 
# plot(mB.rc[[33]])
# 
# save(mB.rc[["mB2017"]], file = "mapBiomas2017_1k.rdata")
# 
# 
# 
