...
# Get absence data from ebird analysis
...
years = seq(min(recordings[,'year'],surveys[,'year']),endYear,by =1)
...
pas.sur.years.lt05 <- years.lt2005[!years.lt2005 %in% surveys[,1] & !years.lt2005 %in% recordings[,1]] #find years without either surveys or recordings
...
pas.sur.years.gte05 <- years.gte2005[!years.gte2005 %in% surveys[,1] & !years.gte2005 %in% recordings[,1]] #find years without
...
surveys = rbind(surveys[,-8],pas.surveys.lt05, pas.surveys.gte05)
...
save(years, PXt, file = "code/sightings.rdata")
